...
# ── TRUST HORIZON CONFIDENCE INSTABILITY THRESHOLD ────────────────────────────

_HIGH_CONFIDENCE_MAX_INSTABILITY = 10.0   # instability must be below this for HIGH confidence
...
def _adjust_trust_horizon(
    ...
    if i_score < _HIGH_CONFIDENCE_MAX_INSTABILITY and barrier_status == BARRIER_CLEAR:
    ...
    if barrier_status == BARRIER_HARD or ced_label == "CRITICAL":
        confidence = "LOW"
          ...
          i_score >= _HIGH_CONFIDENCE_MAX_INSTABILITY):
        confidence = "MODERATE"
    else:
        confidence = "HIGH"
